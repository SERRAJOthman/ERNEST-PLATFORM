// Mocking types to make this testable without full RN install
...
export type UIMode = 'STANDARD' | 'VOICE_HANDS_FREE' | 'PLAN_VIEW' | 'SAFETY_WARNING';
...
export class ContextService {
     ...
     */
    public determineContext(accel: AccelerometerData, location: LocationData): UIMode {
    ...
    private isInZone(userLoc: LocationData, zone: { lat: number; lng: number; radius: number }): boolean {
        // Simple Euclidean distance for mock (not Haversine for performance/simplicity here)
        const dist = Math.sqrt(
            (userLoc.latitude - zone.lat) ** 2 +
